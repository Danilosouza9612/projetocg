...
public class Window extends JFrame implements Observer{
	...
	private String selected = "src//untitled.obj";
	...
	public Window() {
		...
		comboBox.setModel(new DefaultComboBoxModel(new String[] {"Cubo", "Pir\u00E2mide", "Cilindro", "Cone", "Arquivo Externo"}));
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Drawing drawing;
				if(selected.equals("Cubo")) {
					drawing = new Drawing("src//untitled.obj");
				}
				else if(selected.equals("Pir\u00E2mide")) {
					drawing = new Drawing("src//piramide.obj");
				}
				else if(selected.equals("Cilindro")) {
					drawing = new Drawing("src//cilindro.obj");
				}
				else if(selected.equals("Cone")) {
					drawing = new Drawing("src//cone.obj");
				}
				else {
					drawing = new Drawing(textField.getText());
				}
				AppService appService = AppService.getInstance();
		...
		btnNewButton_2.setBounds(10, 462, 110, 23);
		...
		JCheckBox CheckBoxXZ = new JCheckBox("XZ");
		CheckBoxXZ.setSelected(true);
		CheckBoxXZ.setBounds(12, 541, 83, 23);
		CheckBoxXZ.addItemListener(new ItemListener() {
			
			@Override
			public void itemStateChanged(ItemEvent e) {
				if(CheckBoxXZ.isSelected()) {
					AppService appService = AppService.getInstance();
					appService.setShowXZGrid(true);
				}
				else {
					AppService appService = AppService.getInstance();
					appService.setShowXZGrid(false);
				}
				
				
			}
		});
		panel.add(CheckBoxXZ);
		
		JCheckBox CheckBoxXY = new JCheckBox("XY");
		CheckBoxXY.setBounds(97, 541, 87, 23);
		CheckBoxXY.addItemListener(new ItemListener() {
			
			@Override
			public void itemStateChanged(ItemEvent e) {
				if(CheckBoxXY.isSelected()) {
					AppService appService = AppService.getInstance();
					appService.setShowXYgrid(true);
				}
				else {
					AppService appService = AppService.getInstance();
					appService.setShowXYgrid(false);
				}
				
				
			}
		});
		panel.add(CheckBoxXY);
		
		JCheckBox CheckBoxYZ = new JCheckBox("YZ");
		CheckBoxYZ.setBounds(181, 541, 97, 23);
		CheckBoxYZ.addItemListener(new ItemListener() {
			
			@Override
			public void itemStateChanged(ItemEvent e) {
				if(CheckBoxYZ.isSelected()) {
					AppService appService = AppService.getInstance();
					appService.setShowYZgrid(true);
				}
				else {
					AppService appService = AppService.getInstance();
					appService.setShowYZgrid(false);
				}
				
				
			}
		});
		panel.add(CheckBoxYZ);
		
		JLabel lblNewLabel_5 = new JLabel("Ativar / Desativar Grids");
		lblNewLabel_5.setBounds(14, 520, 145, 14);
